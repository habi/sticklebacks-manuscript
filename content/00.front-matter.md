...
*The (anonymized) version [`v1.0`](https://github.com/{{manubot.ci_source.repo_slug}}/releases/tag/v1.0) of this manuscript was submitted to [Methods in Ecology and Evolution](https://besjournals.onlinelibrary.wiley.com/journal/2041210X).*
...
{% endif -%}
on {{manubot.generated_date_long}} and contains {{manubot.manuscript_stats.word_count}} words.
Changes the present version to the submitted version are visualized [here on GitHub](https://github.com/{{manubot.ci_source.repo_slug}}/compare/v1.0...{%- if manubot.ci_source is defined -%}{{manubot.ci_source.commit}}{%- else -%}main{%- endif -%}#files_bucket).
</em></small>
...
[{{ author.name }}](mailto:{{ author.email}}?subject=Feedback%20on%20%22{{pandoc.title}}%22){{ ", " if not loop.last else "." }}
{% endfor %}
{% endif %}
:::
