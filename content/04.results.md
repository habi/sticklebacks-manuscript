...
The reulting image stack, containing six fish in one multiscan, creates a large output file that can be unwieldy to manage even for experienced users.
Depending on the available hardware, it may not even be possible to load in order to manually perform the cropping, such as Fiji [@doi:10.1038/nmeth.2019].
...
Nontheless, variation in the first ceratobranchial was significantly associated with differing ecotypes, although the effect size was small (p = 0.009, R^2 = 0.02056), with a substantial overlap in the resulting shape space (see Figure @fig:pca_cb1).
