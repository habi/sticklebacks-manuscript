...
def resample_curve(
    resampleNumber=31, nodeName="OC_Test", newNodeName="resampledCurveTest"
):
    ...
    resampledCurve = slicer.mrmlScene.AddNewNodeByClass(
        "vtkMRMLMarkupsCurveNode", newNodeName
    )
    ...
    for filename in os.listdir(directory):
        ...
        if filename.endswith('.json'):
            ...
            if "Curve1" in filename or "Curve2" in filename or "Curve5" in filename:
                ...
                resample_curve(
                    21,
                    filename.removesuffix(".mrk.json"),
                    newname.removesuffix(".mrk.json"),
                )
                ...
                "Curve3" in filename
                or "Curve4" in filename
                or "Curve6" in filename
                or "Curve7" in filename
            ):
