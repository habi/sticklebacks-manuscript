...
header-includes: |
  ...
  <meta name="dc.modified" content="2026-09-25T20:09:31+00:00" />
  <meta property="article:modified_time" content="2026-09-25T20:09:31+00:00" />
  ...
  <link rel="alternate" type="text/html" href="https://habi.github.io/sticklebacks-manuscript/v/5704138892041401f344c533286376e460901774/" />
  <meta name="manubot_html_url_versioned" content="https://habi.github.io/sticklebacks-manuscript/v/5704138892041401f344c533286376e460901774/" />
  <meta name="manubot_pdf_url_versioned" content="https://habi.github.io/sticklebacks-manuscript/v/5704138892041401f344c533286376e460901774/manuscript.pdf" />
  <meta property="og:type" content="article" />
...
<small><em>
This manuscript
([permalink](https://habi.github.io/sticklebacks-manuscript/v/5704138892041401f344c533286376e460901774/))
was generated
from [habi/sticklebacks-manuscript@5704138](https://github.com/habi/sticklebacks-manuscript/tree/5704138892041401f344c533286376e460901774)
on September 25, 2026 and contains 5375 words.
Changes to the submitted version are visualized [here on GitHub](https://github.com/habi/sticklebacks-manuscript/compare/v1.0...5704138892041401f344c533286376e460901774#files_bucket).
</em></small>
...
However, the resulting increase in data complexity and processing demands often makes these approaches impractical for large-scale studies, especially at population scales that require measuring and analyzing hundreds to thousands of samples.

To address these challenges, we developed a rapid and semiautomated segmentation and analysis pipeline based on the Jupyter interactive development environment and the Biomedisa image segmentation platform to investigate three-dimensional morphological adaptation in the threespine stickleback (*Gasterosteus aculeatus*).

The pipeline includes separation of multispecimen scans into individual regions of interest, reconstruction of targeted anatomical structures, and morphometric analyses.
We applied this pipeline to 38 multispecimen scans comprising 215 specimens from divergent benthic and limnetic stickleback populations, demonstrating the potential of a high-throughput micro-CT imaging approach for testing ecological and evolutionary hypotheses.
...
Among sticklebacks and other bony fish, differences in jaw structures are directly related to functional and kinematic differences between ecotypes [@doi:10.1002/ece3.6929].
...
Moreover, these structures are difficult to study without full cranial dissection and the corresponding distortion of the branchial anatomy.
...
To optimize for these constraints, we scanned all sticklebacks in batches of six fish in a custom-made 3D-printed sample holder per scan.
This holder was designed in [OpenSCAD](https://openscad.org/) (OpenSCAD Developers; version 2021.01) and is available online, either directly as an [STL file for printing](https://github.com/TomoGraphics/Hol3Drs/blob/master/STL/Stickleback.Multiple.stl) or as a [(parameterized) OpenSCAD file](https://github.com/TomoGraphics/Hol3Drs/blob/master/Stickleback.Multiple.scad) for adaptation to other classes of samples.
...
Because of the length of the fish, we had to acquire so-called stacked scans; on average, we scanned three fields of view along the rotation axis of the sample holder.
This resulted in an average scan time of 10 hours for each scan.
The projection images were subsequently reconstructed into stacks of 8-bit `png` images with NRecon (Bruker microCT, Kontich, Belgium; version 2.2.0.6), without applying any ring artifact or beam hardening correction.
The isotropic voxel sizes in the resulting datasets vary from 15 to 17.5 µm.

The result of one such multispecimen scan is visualized in Figure @fig:visualization, which shows the six fish contained in this scan.
The labels on the holder are colored red and are shown extracted (in yellow) later in Figure @fig:labels.

![Three-dimensional visualization of one multispecimen scan.
...
Further processing of the tomographic datasets was performed with a set of Jupyter [@doi:10.3233/978-1-61499-649-1-87] notebooks [@doi:10.5281/zenodo.18257528].

##### Preview Notebook

The [preview notebook](https://nbviewer.org/github/habi/sticklebacks/blob/main/PreviewScans.ipynb) was used to identify scanning and reconstruction issues.
...
The [separation notebook](https://nbviewer.org/github/habi/sticklebacks/blob/main/BucketSeparator.ipynb) processed all the acquired scans to extract each individual fish from each scan that encompasses six fish in total.
As in the preview notebook, we efficiently loaded all the `png`s from disk with [`dask`](https://www.dask.org/) [@dask].
Based on the previously extracted MIP images and a simple labeling of these images ([`skimage.measure.label`](https://scikit-image.org/docs/stable/api/skimage.measure.html#skimage.measure.label)), we extracted both the labels in the custom-made sample holder and the positions of individual fish in the scan ([`skimage.measure.regionprops`](https://scikit-image.org/docs/stable/api/skimage.measure.html#skimage.measure.regionprops)) (see Figure @fig:labels).
...
Based on a simple mapping of the detected regions to the ID numbers of the scanned fish, we labeled the resulting images and presented these images together with photos of the lab book and sample tubes for verification (see Figure @fig:checking).
...
As a first step, we wrote the regions of the single fish to disk in `zarr` [@doi:10.5281/zenodo.3773450] format, which is well suited to storing n-dimensional arrays on disk.
...
All these log files are also available in the processing repository; an example of such a log file (from the region shown in Figure @fig:SLX23012_3D) is [shown online here](https://github.com/habi/sticklebacks/blob/main/logfiles/BucketOfFish_H/rec_regions/SL.X23.012/SL.X23.012.log).
...
For this further analysis, we also wrote stacks of `png` images and [`nrrd`](https://teem.sourceforge.net/nrrd/) files for each fish region in both cropped and cropped-and-binarized forms.
These binarized regions were segmented into bone and background based on a simple multi-Otsu thresholding algorithm [@doi:10.6688/JISE.2001.17.5.1] (with four classes, using only the middle threshold).
...
`napari` is well suited to load the intermediate `*.zarr` files; Figure @fig:SLX23012_3D shows such a preview.
...
The model was trained with a batch size of 24 for 50 epochs, using a network architecture of 32-64-128-256-512.
The final model performs well, with a Dice score of 0.9159 on the validation dataset.
We only performed manual touch-ups where the adjacent bones were in close proximity (causing their appearance to be "stuck" in the final render; this is also an issue with manual segmentation and not exclusive to the approach described here).
...
Type II landmarks were placed at the ends of each bone, with semilandmarks between them to cover the curvature axes along the bone (see Figure @fig:landmarks).
In total, seven landmarks and four semilandmark curves (two containing 20 semilandmarks, two containing 15) were placed on the first ceratobranchial (CB1), and five landmarks and three semilandmark curves (one containing 20 semilandmarks, two containing 15) were placed on the second ceratobranchial (CB2).
...
The extraction based on the MIP image along the rotation axis is fully automated, reproducible, and robust, since the detected fish "regions" do not overlap in the resulting image.

The resulting image stack, containing six fish in one multispecimen scan, creates a large output dataset that can be unwieldy to manage even for experienced users.
Depending on the available hardware, it may not even be possible to load the resulting image stack into image processing software to manually perform the cropping.
Large stacks of images (i.e., larger than the RAM of the machine used) can be loaded as [virtual stacks](https://imagej.net/ij/plugins/virtual-opener.html), but to manually crop the region of each fish from the large scan with the [Crop (3D)](https://www.longair.net/edinburgh/imagej/three-pane-crop/) function, one needs to load the full dataset.
Since one (exemplary) dataset (`Sticklebucket_10`) is 7 GB on disk and reported as 35.4 GB when loaded in Fiji [@doi:10.1038/nmeth.2019], using the 3D cropping function on an original single multispecimen dataset is not possible without a powerful workstation.

Extracting individual fish from the encompassing dataset would thus be a two-step manual process, e.g., cropping the full dataset loaded as a virtual stack and then cropping it down further before writing out the cropped stack.
For each encompassing scan, this would need to be repeated six times (for *each* of the six fish in each of the encompassing scans).
...
Algorithmically/automatically cropping the large datasets based on the axial MIP image yields reproducible cropped regions and uses operator time efficiently by eliminating manual cropping steps (see Table @tbl:timing).

| Task                             | Est. Manual Time |      Pipeline Time | Speedup |
|----------------------------------|-----------------:|-------------------:|--------:|
| Scanning single fish             |        ~10 hours | 1 hour, 45 minutes |   ~5.5× |
| Splitting scans into single fish |       17 minutes |          9 minutes |     ~2× |
| Rendering volumes                |        5 minutes |         15 seconds |     20× |
| Segmentation                     |   10--15 minutes |         15 seconds |    ~50× |
...
Table: Comparison of time estimates between manual and pipeline runs.
       ...
       The manual scan time per single fish is thus calculated to be five times longer than scanning six fish at a time in a single scan and separating them after the fact.
...
Note that the automated extraction writes intermediate files, which facilitate the handling of the data (this step takes about 3 minutes per scan).
...
Selection and individual rendering of branchial structures takes between 10 and 15 minutes; the segmentation of an average Biomedisa render takes 15 seconds once trained (see Table @tbl:timing).

### Analysis

The speed and quality of these data allow us to study the internal branchial anatomy on a large scale and in situ, without the need for fine dissection.

After GPA alignment, we quantified the shape differences among all scanned fish for this project.
Changes due to allometry (using centroid size or standard length as the size metric) were significant but slight, explaining only a small fraction of shape variation (CB1: *p* = 0.001, $R^2$ = 0.03246; CB2: *p* = 0.001, $R^2$ = 0.06220) in both bones.
...
On the other hand, the second ceratobranchial bone showed equally small but significant changes associated with the ecotype (*p* = 0.001, $R^2$ = 0.0377).
...
The differences in the second ceratobranchial appear to be driven by divergence in the South Rolly population, supported by significant pairwise differences between this lake and all other lakes in CB2 but not in CB1 (see [Supplementary Materials]).
...
Requiring minimal active user input, all automated pipeline steps are substantially faster than equivalent manual processing by an expert operator.
...
CB1 showed statistically significant differences between benthic and limnetic ecotypes based on linear and pairwise analyses (and PC4, accounting for 4.33% of the variation, appears to capture these differences; see the supplementary figures).
...
The increased surface area observed in benthic fish may provide a greater area for muscle attachment, which could enhance the ability of fish to abduct these structures during water filtration [@doi:10.1007/s10228-004-0251-5; @hdl:2268/14698].
...
The different responses of the first and second ceratobranchials raise the possibility of a modular response to dietary changes within the branchial basket.
...
As with other multispecimen scanning projects, the scanning parameters can be optimized individually for each scan but not for each individual specimen.
...
Extracting individual specimens from multispecimen scans is efficient; a custom 3D-printed holder and automated splitting pipeline minimize operator time and maximize throughput.
...
In its current implementation, Biomedisa is on average 50 times faster than manual segmentation by a skilled operator, and without the interoperator variability inherent to manually segmenting large numbers of specimens.
This puts virtual, nondestructive dissection of internal stickleback anatomy on par with, if not beyond, hand dissection.

Finally, the 3D analysis step of the pipeline allows for insights from 3D data that cannot be obtained from traditional dissection, including complex shapes and arrangements not accessible through destructive sampling approaches.
...
The CSV file [ScanningDetails.csv](https://github.com/habi/sticklebacks-manuscript/blob/main/content/data/ScanningDetails.csv) gives a tabular overview of all the (relevant) parameters of all the scans we performed.
...
All Jupyter notebooks used to process the acquired tomographic data as described in the text are available [on GitHub](https://github.com/habi/sticklebacks) and can be previewed with [nbviewer](https://nbviewer.org/github/habi/sticklebacks/tree/main/).
...
The Python script we used to resample curves is found [online on GitHub](https://github.com/habi/sticklebacks-manuscript/blob/main/content/scripts/Resample_Curve.py).
